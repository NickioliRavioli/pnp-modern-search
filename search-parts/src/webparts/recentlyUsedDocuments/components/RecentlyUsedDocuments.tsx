...
import { DocumentCard, DocumentCardPreview, IDocumentCardPreviewProps } from '@fluentui/react/lib/DocumentCard';
...
export default class RecentlyUsedDocuments extends React.Component<IRecentlyUsedDocumentsProps, IRecentlyUsedDocumentsState> {
  ...
  private _fetchRecentDocuments() {
    if (this.props.graphClient) {
      ...
      // and resourceVisualization/containerDisplayName eq 'CDC'`)
      // and startsWith(resourceReference/webUrl, 'https://dbctcomau.sharepoint.com/sites/CDC')`)
      // .search('"resourceVisualization/containerWebUrl:https://dbctcomau.sharepoint.com/sites/CDC"')
      // and startsWith(resourceVisualization/containerWebUrl, 'https://dbctcomau.sharepoint.com/sites/CDC')`)

      .get((err: { message: string; }, res: IRecentDocuments) => {
        console.log("siteFilter", this.props.siteFilter);
  ...
  private _processRecentDocuments(recentDocs: IRecentDocument[]): void {
    ...
    this.setState({
      ...
      recentDocs: recentDocs.filter(x => x.resourceVisualization.containerWebUrl.startsWith(this.props.siteFilter)).slice(0, this.props.nrOfItems ? this.props.nrOfItems : 10),
      loading: false
    });
  }
    ...
    return (
      <div className={styles.document}>
        <DocumentCard 
          ...
          onClickHref={item.resourceReference.webUrl}
          type={DocumentCardType.compact}>

          <div className={styles.documentDetails}>

              <div className={styles.title}>{item.resourceVisualization.title}</div>
              <div >{`${strings.LastUsedMsg} ${this._relativeDate(item.lastUsed.lastAccessedDateTime)}`}</div>
              <div className={styles.previewText}>{item.resourceVisualization.previewText}</div>
  ...
  public componentDidUpdate(prevProps: IRecentlyUsedDocumentsProps, prevState: IRecentlyUsedDocumentsState): void {
    if (prevProps.nrOfItems !== this.props.nrOfItems ||
      prevProps.siteFilter !== this.props.siteFilter) {
      this._fetchRecentDocuments();
    }
  }
